...
b<-paste0("./Data/wrfout_d01_",seq(2020,2070,1),".nc") # List of names to save locally as
...
system.time(future_map2(a,b,dl)) # In RStudio, this may hang even though all files are downloaded
...
df<-nc_open("./Data/wrfout_d01_2020.nc")
print(df)
...
system.time(xyz2<-xyz2 %>% 
  filter(.,hub==109)) # Keeping only data for hub-height of 109m, will interpolate
...
multiweibull<-function(param3){
  xyz2<-xyz2 %>% filter(llindex == param3) # Subset for each point
